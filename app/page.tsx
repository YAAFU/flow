...
export default function Home(){
  ...
  const saveNewTask=async(task:Task,repeat:RepeatDraft={frequency:"none"})=>{if(taskFlow?.mode!=="addingTask")throw new Error("ไม่มีวันที่สำหรับงานใหม่");const date=taskFlow.date;const isFirstTask=totalTaskCount===0;addTaskForDate(date,task,repeat);if(isFirstTask)trackProductEvent("first_task_created",{entryPoint:"today"});showDate(date);setDayView("list");setTaskFlow(null);};
  const saveEditedTask=async(task:Task)=>{if(taskFlow?.mode!=="editingTask")throw new Error("ไม่พบงานที่กำลังแก้ไข");const date=taskFlow.date;updateFlow(previous=>({...previous,selectedDate:date,tasksByDay:updateTaskInDate(previous.tasksByDay,date,task)}));showDate(date);setDayView("list");setTaskFlow(null);};
  ...
  const applyPlannerPlan=(input:PlannerApplyInput)=>{const now=new Date().toISOString();const merged=mergeScheduleIntoTasks({existing:input.allCurrentTasks,drafts:input.drafts,schedule:input.plan.plans[input.variant].schedule,categories:state.categories,now,createId:()=>uid("ai-plan")});commit(previous=>{const withRecurrence=attachPlannerRecurrences(input.targetDate,merged.tasks,merged.includedDrafts,previous.recurrenceRules,now);return{...previous,selectedDate:input.targetDate,tasksByDay:{...previous.tasksByDay,[input.targetDate]:withRecurrence.tasks.map(task=>TaskSchema.parse(task))},recurrenceRules:withRecurrence.rules};},`ใช้แผน ${input.variant} แล้ว`);revealPlannerDate(input.targetDate);};
  ...
  const reopenTour=(mode:GuidedTourMode)=>{setTourMode(mode);trackProductEvent("tour_reopened",{entryPoint:"settings"});};
  ...
  const resetGuidance=()=>{resetOnboardingState(window.localStorage);setGuideNotice("รีเซ็ตคำแนะนำแล้ว งานและการตั้งค่าเดิมยังอยู่ครบ");};
  ...
  return <main className="min-h-dvh bg-[var(--flow-paper)] pb-[calc(7.5rem+env(safe-area-inset-bottom))] pt-[env(safe-area-inset-top)] text-[var(--flow-ink)]"><PwaRegister/><div className="mx-auto w-full max-w-[420px] px-4 pt-5">
    ...
    {tab==="today"&&<>
      ...
      <div key={`${dayView}-${selectedDate}`} className="flow-view">
        ...
        {dayView==="list"&&shownTasks.length===0&&tasks.length>0?<section className="flow-surface rounded-2xl border border-dashed border-[var(--flow-line)] p-5 text-center"><h2 className="font-bold">ยังไม่มีงานในหมวดนี้</h2><p className="mt-1 text-sm text-[var(--flow-muted)]">เลือก “ทั้งหมด” เพื่อดูแผนของวันนี้</p><button type="button" onClick={()=>setCategoryFilter("")} className="flow-press mt-3 min-h-11 rounded-xl border border-[var(--flow-line)] px-4 text-sm font-semibold">ดูงานทั้งหมด</button></section>:null}
        {dayView==="list"&&!(shownTasks.length===0&&tasks.length>0)&&<TaskListView tasks={shownTasks} categories={state.categories} onToggle={toggleTask} onEdit={openTaskEditor} onDelete={removeTask} onAdd={()=>setTaskFlow({mode:"addingTask",date:selectedDate})} onTryExample={()=>router.push("/guide?start=templates")} onOpenGuide={()=>router.push("/guide?restart=1")}/>}
  ...
  {hydrated&&tourMode&&<GuidedTour active mode={tourMode} onNavigate={navigateTour} onComplete={()=>setTourMode(null)} onSkip={()=>setTourMode(null)} onClose={()=>setTourMode(null)} returnFocus={()=>document.querySelector<HTMLElement>('nav[aria-label="เมนูหลัก"] button[aria-current="page"]')}/>}
