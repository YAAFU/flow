...
import { BarChart3, CalendarDays, ChevronLeft, ChevronRight, Clock3, LayoutList, ListTodo, LogOut, Map as MapIcon, Plus, Search, Settings, Sparkles, Undo2, WifiOff } from "lucide-react";
import { GuidedTour, type GuidedTourMode, type GuidedTourView } from "@/components/GuidedTour";
import type { MapTaskItem } from "@/components/FlowMap";
...
import { hasCoordinates, taskLocationFromFlat, taskLocationToFlat } from "@/lib/location";
import { bangkokPlaceHour, buildSavedPlace, findSavedPlaceMention, rankPlaceSuggestions, recordRecentPlace, savedPlaceToLocation } from "@/lib/smart-places";
...
import { PlanResultSchema, TaskSchema, type FlowState, type PlanResult, type RecentPlace, type RecurrenceRule, type SavedPlace, type Task } from "@/lib/types";
import { useFlowStore } from "@/hooks/useFlowStore";

type Tab = "today" | "calendar" | "dashboard" | "search" | "settings";
type DayView = "list" | "timeline" | "map";
...
export default function Home(){
  ...
  const mapItems=useMemo<MapTaskItem[]>(()=>tasks.map(task=>{
    const scheduled=Boolean(task.fixedTime&&!task.allDay);
    const start=scheduled?task.fixedTime:undefined;
    return{
      taskId:task.id,
      title:task.title,
      placeLabel:task.place.trim()||undefined,
      lat:task.lat,
      lng:task.lng,
      start,
      end:start&&task.durationMin?endTime(start,task.durationMin):undefined,
      scheduled,
    };
  }),[tasks]);
  ...
  const locationSuggestions=useMemo(()=>rankPlaceSuggestions(state.savedPlaces,state.recentPlaces,{date:selectedDate,hour:bangkokPlaceHour()},state.settings.suggestFrequentPlaces&&state.settings.useLocationHistory),[selectedDate,state.recentPlaces,state.savedPlaces,state.settings.suggestFrequentPlaces,state.settings.useLocationHistory]);
  const promoteRecentPlace=(recent:RecentPlace)=>{const name=recent.placeName.trim().toLocaleLowerCase("th-TH");if(state.savedPlaces.some(place=>place.label.trim().toLocaleLowerCase("th-TH")===name||place.placeName.trim().toLocaleLowerCase("th-TH")===name))return;const place=buildSavedPlace({label:recent.placeName,placeName:recent.placeName,latitude:recent.latitude,longitude:recent.longitude,category:"custom"});updateFlow(previous=>({...previous,savedPlaces:[...previous.savedPlaces,place]}));trackProductEvent("saved_place_created",{category:"custom",hasCoordinates:recent.latitude!=null&&recent.longitude!=null});};
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{
      ...
      if(requestedView==="timeline"||requestedView==="list"||requestedView==="map"){
        setTab("today");
        setDayView(requestedView);
      }else if(requestedView==="focus"){
        setTab("today");
        setDayView("timeline");
      }
  ...
  const addTaskForDate=(date:string,task:Task,repeat:RepeatDraft)=>{const usedAt=new Date();const now=usedAt.toISOString();commit(previous=>{const recurrenceRules=[...previous.recurrenceRules];let nextTask=task;if(repeat.frequency!=="none"){const ruleId=uid("series");const dateValue=dateParts(date);const weekday=new Date(Date.UTC(dateValue.year,dateValue.month,dateValue.day)).getUTCDay();recurrenceRules.push({id:ruleId,taskId:task.id,frequency:repeat.frequency,interval:1,weekdays:repeat.frequency==="weekly"?[weekday]:[],startDate:date,excludedDates:[],createdAt:now,updatedAt:now});nextTask={...task,seriesId:ruleId,occurrenceDate:date};}const location=taskLocationFromFlat(nextTask);return{...previous,selectedDate:date,tasksByDay:addTaskToDate(previous.tasksByDay,date,TaskSchema.parse(nextTask)),recurrenceRules,recentPlaces:previous.settings.useLocationHistory?recordRecentPlace(previous.recentPlaces,location,{usedAt,categoryId:nextTask.categoryId}):previous.recentPlaces};},"เพิ่มงานแล้ว");};
  ...
  const saveNewTask=async(task:Task,repeat:RepeatDraft={frequency:"none"})=>{if(taskFlow?.mode!=="addingTask")throw new Error("ไม่มีวันที่สำหรับงานใหม่");const date=taskFlow.date;const isFirstTask=totalTaskCount===0;addTaskForDate(date,task,repeat);const location=taskLocationFromFlat(task);if(location&&location.source!=="quick"){trackProductEvent("location_selected",{source:location.source,hasCoordinates:hasCoordinates(location)});if(location.source==="suggested")trackProductEvent("location_suggestion_used",{source:"suggested",hasCoordinates:hasCoordinates(location)});}if(isFirstTask)trackProductEvent("first_task_created",{entryPoint:"today"});if(quickStart.status==="started"&&quickStart.stage==="add_task"){const next=recordQuickStartTask(window.localStorage,task.id,Boolean(task.fixedTime));setQuickStart(next.quickStart);trackProductEvent("quick_start_task_created");if(task.fixedTime)setQuickStartSuccessOpen(true);}showDate(date);setDayView("list");setTaskFlow(null);};
  const saveEditedTask=async(task:Task)=>{if(taskFlow?.mode!=="editingTask")throw new Error("ไม่พบงานที่กำลังแก้ไข");const date=taskFlow.date;const usedAt=new Date();updateFlow(previous=>({...previous,selectedDate:date,tasksByDay:updateTaskInDate(previous.tasksByDay,date,task),recentPlaces:previous.settings.useLocationHistory?recordRecentPlace(previous.recentPlaces,taskLocationFromFlat(task),{usedAt,categoryId:task.categoryId}):previous.recentPlaces}));if(quickStart.status==="started"&&quickStart.stage==="schedule_task"&&quickStart.taskId===task.id&&task.fixedTime){const next=recordQuickStartTask(window.localStorage,task.id,true);setQuickStart(next.quickStart);setQuickStartSuccessOpen(true);}showDate(date);setDayView("list");setTaskFlow(null);};
  ...
  const parsePlannerInput=async(input:PlannerParseInput):Promise<ParsedTasksResponse>=>{let parsed:ParsedTasksResponse;try{parsed=ParsedTasksResponseSchema.parse(await requestAiJson("/api/parse",{text:input.text,nowIso:new Date().toISOString(),timezone:"Asia/Bangkok",selectedDate:input.targetDate},25_000));}catch(error){if(error instanceof Error&&error.name==="AbortError")throw error;parsed=ParsedTasksResponseSchema.parse({tasks:buildLocalParsedTasks(input.text,input.targetDate),mode:"local",date:input.targetDate,warnings:["ไม่สามารถเชื่อมต่อระบบวิเคราะห์ข้อความได้ จึงใช้ตัวแยกข้อความภายในแทน"]});}const tasks=parsed.tasks.map(task=>{const mention=findSavedPlaceMention(`${task.sourceText??""} ${task.title} ${task.place??""}`,state.savedPlaces);if(!mention)return task;const currentPlace=(task.place??"").trim().toLocaleLowerCase("th-TH");const matchesPlace=!currentPlace||currentPlace===mention.label.toLocaleLowerCase("th-TH")||currentPlace===mention.placeName.toLocaleLowerCase("th-TH")||`${task.sourceText??""} ${task.title}`.includes(mention.label);if(!matchesPlace)return task;return{...task,...taskLocationToFlat(savedPlaceToLocation(mention)),reviewReason:[task.reviewReason,`จับคู่กับสถานที่ประจำ “${mention.label}” ในเครื่องนี้`].filter(Boolean).join(" · ")};});return ParsedTasksResponseSchema.parse({...parsed,tasks});};
  const plannerTasks=(input:PlannerGenerateInput)=>{const now=new Date().toISOString();const current=input.currentTasks.filter(task=>task.durationMin!=null);const eligibleDrafts=input.drafts.filter(draft=>(draft.date??input.targetDate)===input.targetDate&&draft.durationMin!=null);const draftTasks=eligibleDrafts.map((draft,index)=>plannerDraftToTask(draft,current.length+index,state.categories,now));const ids=new Set(current.map(task=>task.id));for(const task of draftTasks){if(ids.has(task.id))throw new Error("พบงานฉบับร่างซ้ำ กรุณาสร้างฉบับร่างใหม่");ids.add(task.id);}return[...current,...draftTasks];};
  ...
  const revealPlannerDate=(date:string)=>{showDate(date);setDayView("timeline");updateFlow(previous=>previous.selectedDate===date?previous:{...previous,selectedDate:date});};
  const appendAiDrafts=({targetDate,drafts}:PlannerAppendInput)=>{const usedAt=new Date();const now=usedAt.toISOString();const firstDate=drafts[0]?.date??targetDate;commit(previous=>{let tasksByDay={...previous.tasksByDay};let rules=[...previous.recurrenceRules];let recentPlaces=[...previous.recentPlaces];const grouped=new Map<string,PlannerDraftTask[]>();for(const draft of drafts){const date=draft.date??targetDate;grouped.set(date,[...(grouped.get(date)??[]),draft]);if(previous.settings.useLocationHistory)recentPlaces=recordRecentPlace(recentPlaces,taskLocationFromFlat(draft),{usedAt});}for(const [date,items] of grouped){const appended=appendPlannerDrafts(tasksByDay[date]??[],items,previous.categories,now);const withRecurrence=attachPlannerRecurrences(date,appended,items,rules,now);tasksByDay={...tasksByDay,[date]:withRecurrence.tasks.map(task=>TaskSchema.parse(task))};rules=withRecurrence.rules;}return{...previous,selectedDate:firstDate,tasksByDay,recurrenceRules:rules,recentPlaces};},"เพิ่มงานจากฉบับร่างแล้ว");revealPlannerDate(firstDate);};
  const applyPlannerPlan=(input:PlannerApplyInput)=>{const usedAt=new Date();const now=usedAt.toISOString();const schedule=input.plan.plans[input.variant].schedule;const targetDrafts=input.drafts.filter(draft=>(draft.date??input.targetDate)===input.targetDate);const otherDrafts=input.drafts.filter(draft=>(draft.date??input.targetDate)!==input.targetDate);const merged=mergeScheduleIntoTasks({existing:input.allCurrentTasks,drafts:targetDrafts,schedule,categories:state.categories,now,createId:()=>uid("ai-plan")});commit(previous=>{const tasksByDay={...previous.tasksByDay};let withRecurrence=attachPlannerRecurrences(input.targetDate,merged.tasks,merged.includedDrafts,previous.recurrenceRules,now);tasksByDay[input.targetDate]=withRecurrence.tasks.map(task=>TaskSchema.parse(task));let rules=withRecurrence.rules;let recentPlaces=[...previous.recentPlaces];const grouped=new Map<string,PlannerDraftTask[]>();for(const draft of input.drafts){if(previous.settings.useLocationHistory)recentPlaces=recordRecentPlace(recentPlaces,taskLocationFromFlat(draft),{usedAt});}for(const draft of otherDrafts){const date=draft.date!;grouped.set(date,[...(grouped.get(date)??[]),draft]);}for(const [date,items] of grouped){const appended=appendPlannerDrafts(tasksByDay[date]??[],items,previous.categories,now);withRecurrence=attachPlannerRecurrences(date,appended,items,rules,now);tasksByDay[date]=withRecurrence.tasks.map(task=>TaskSchema.parse(task));rules=withRecurrence.rules;}return{...previous,selectedDate:input.targetDate,tasksByDay,recurrenceRules:rules,recentPlaces};},`ใช้แผน ${input.variant} แล้ว`);if(quickStart.status==="started"&&quickStart.stage==="schedule_task"&&quickStart.taskId&&schedule.some(item=>item.taskId===quickStart.taskId)){const next=recordQuickStartTask(window.localStorage,quickStart.taskId,true);setQuickStart(next.quickStart);quickStartSuccessPendingRef.current=true;trackProductEvent("quick_start_plan_applied",{plannerMode:input.plan.mode??"local"});}revealPlannerDate(input.targetDate);};
  ...
  const navigateTour=(view:GuidedTourView)=>{
    ...
    if(view==="timeline"){setTab("today");setDayView(view);return;}
    setTab(view);
  };
  ...
  const openQuickStartTask=()=>{setTourMode(null);setQuickStartSuccessOpen(false);setTab("today");setDayView("list");setTaskFlow({mode:"addingTask",date:selectedDate});};
  const openQuickStartPlanner=()=>{setTourMode(null);setTab("today");setAiOpen(true);trackProductEvent("quick_start_planner_opened");};
  const openBulkTextParser=()=>{setTaskFlow(null);setTourMode(null);setAiOpen(true);trackProductEvent("bulk_text_parser_opened");};
  const openTimeline=()=>{setTab("today");setDayView("timeline");window.requestAnimationFrame(()=>dayViewAnchorRef.current?.scrollIntoView({block:"start",behavior:"smooth"}));};
  ...
  return <main className="min-h-dvh bg-[var(--flow-paper)] pb-[calc(7.5rem+env(safe-area-inset-bottom))] pt-[env(safe-area-inset-top)] text-[var(--flow-ink)]"><PwaRegister/><div className="mx-auto w-full max-w-[420px] px-4 pt-5">
    ...
    {tab==="today"&&<>
      ...
      {hasScheduledTasks&&<TodayPulse date={selectedDate} tasks={tasks} startHour={state.settings.timelineStartHour} endHour={state.settings.timelineEndHour} onOpenTimeline={openTimeline}/>}
  {tasks.length>0&&<section data-tour="ai-planner" data-quick-start="schedule-task" aria-labelledby="flow-planner-heading" className={`flow-card overflow-hidden rounded-[22px] ${hasScheduledTasks?"my-4":"mb-4"}`}><div className="border-b flow-hairline px-5 py-4"><div className="flex items-center justify-between gap-3"><p className="font-grotesk text-[10px] font-bold tracking-[0.14em] text-[var(--flow-lime-dark)]">FLOW PLANNER</p><span className="rounded-full bg-[var(--flow-lime)] px-2.5 py-1 text-[10px] font-semibold text-[#111111]">{online?"ใช้ AI หรือจัดในเครื่อง":"จัดในเครื่อง"}</span></div><h2 id="flow-planner-heading" className="mt-2 text-xl font-bold">{hasScheduledTasks?"อยากปรับแผนของวันนี้?":"มีงานแล้ว ให้ Flow ช่วยจัดเวลา"}</h2><p className="mt-1 text-sm leading-6 text-[var(--flow-muted)]">Flow จะเสนอเวลา ตรวจช่วงชน และให้คุณเลือกแผนก่อนบันทึกจริง</p></div><div className="p-3"><button data-tour={hasScheduledTasks?undefined:"primary-action"} type="button" onClick={()=>setAiOpen(true)} className={`flow-press flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl px-4 font-semibold ${hasScheduledTasks?"border-[1.5px] border-[var(--flow-line)]":"flow-inverse"}`}><Sparkles size={18} aria-hidden className="text-[var(--flow-lime-dark)]"/>{hasScheduledTasks?"จัดแผนใหม่":"ให้ Flow จัดวัน"}</button><p className="mt-2 text-center text-xs text-[var(--flow-muted)]">ถ้า AI ใช้ไม่ได้ ระบบจะจัดแผนในเครื่องให้ต่อได้ทันที</p></div></section>}
      ...
      <div key={`${dayView}-${selectedDate}`} className="flow-view">
        ...
        {dayView==="map"&&<FlowMap items={mapItems} online={online} onEditTask={openTaskEditor} onAddTask={()=>setTaskFlow({mode:"addingTask",date:selectedDate})}/>}
      </div>
    </>}
  ...
  {tab==="settings"&&<SettingsPanel state={state} onReplace={next=>{localStorage.setItem("flow_state_backup_v1",JSON.stringify(state));replaceFlow(next);}} onSettings={settings=>updateFlow(previous=>({...previous,settings}))} onAddCategory={name=>updateFlow(previous=>({...previous,categories:[...previous.categories,{id:uid("category"),name,createdAt:new Date().toISOString()}]}))} onDeleteCategory={id=>commit(previous=>({...previous,categories:previous.categories.filter(category=>category.id!==id),tasksByDay:Object.fromEntries(Object.entries(previous.tasksByDay).map(([date,items])=>[date,items.map(task=>task.categoryId===id?{...task,categoryId:undefined}:task)]))}),"ลบหมวดหมู่แล้ว")} onSavePlace={(place:SavedPlace)=>{updateFlow(previous=>({...previous,savedPlaces:[...previous.savedPlaces.filter(item=>item.id!==place.id),place]}));trackProductEvent(state.savedPlaces.some(item=>item.id===place.id)?"saved_place_updated":"saved_place_created",{category:place.category,hasCoordinates:place.latitude!=null&&place.longitude!=null});}} onDeletePlace={id=>{const place=state.savedPlaces.find(item=>item.id===id);updateFlow(previous=>({...previous,savedPlaces:previous.savedPlaces.filter(item=>item.id!==id)}));if(place)trackProductEvent("saved_place_deleted",{category:place.category,hasCoordinates:place.latitude!=null&&place.longitude!=null});}} onClearRecentPlaces={()=>{updateFlow(previous=>({...previous,recentPlaces:[]}));trackProductEvent("recent_locations_cleared");}} onStartCoreTour={()=>reopenTour("core")} onStartFullTour={()=>reopenTour("full")} onTrySampleDay={()=>router.push("/guide?start=templates")} onShowCalendarComparison={()=>router.push("/guide?start=comparison")} onRestartGuide={()=>router.push("/guide?restart=1")} onRestartQuickStart={restartQuickStartFromSettings} onResetGuidance={resetGuidance}/>}
  ...
  {taskFlow?.mode==="addingTask"&&<Modal title={`งานใหม่ · ${formatThaiTaskDate(taskFlow.date)}`} description={`เพิ่มงานสำหรับวันที่ ${formatThaiTaskDate(taskFlow.date)}`} onClose={()=>setTaskFlow(null)} returnFocusRef={addTaskTriggerRef}><TaskInput key={`add-${taskFlow.date}`} date={taskFlow.date} categories={state.categories} quickLocations={[]} savedPlaces={state.savedPlaces} recentPlaces={state.settings.useLocationHistory?state.recentPlaces:[]} locationSuggestions={locationSuggestions} locationRecommendationsEnabled={state.settings.suggestFrequentPlaces&&state.settings.useLocationHistory} order={(state.tasksByDay[taskFlow.date]??[]).length} onAdd={saveNewTask} onPromoteRecentPlace={state.settings.promptSaveFrequentPlaces?promoteRecentPlace:undefined} onClearRecentPlaces={()=>updateFlow(previous=>({...previous,recentPlaces:[]}))} onOpenBulk={openBulkTextParser}/></Modal>}
  {taskFlow?.mode==="editingTask"&&editingTask&&<Modal title={`แก้ไขงาน · ${formatThaiTaskDate(taskFlow.date)}`} description={`แก้ไขรายละเอียด ${editingTask.title}`} onClose={()=>setTaskFlow(null)}><TaskInput key={`edit-${editingTask.id}`} date={taskFlow.date} categories={state.categories} quickLocations={[]} savedPlaces={state.savedPlaces} recentPlaces={state.settings.useLocationHistory?state.recentPlaces:[]} locationSuggestions={locationSuggestions} locationRecommendationsEnabled={state.settings.suggestFrequentPlaces&&state.settings.useLocationHistory} editing={editingTask} order={editingTask.order} onAdd={saveNewTask} onSave={saveEditedTask} onCancel={()=>setTaskFlow(null)} onPromoteRecentPlace={state.settings.promptSaveFrequentPlaces?promoteRecentPlace:undefined} onClearRecentPlaces={()=>updateFlow(previous=>({...previous,recentPlaces:[]}))}/></Modal>}
