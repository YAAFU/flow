...
export function TodayPulse({ date, tasks, startHour, endHour, onOpenTimeline, onStartFocus }: {
  date: string;
  tasks: Task[];
  startHour: number;
  endHour: number;
  ...
  return (
    <section aria-labelledby="today-pulse-heading" className="flow-inverse flow-sheet overflow-hidden rounded-[22px] border-[1.5px] border-[#111111] shadow-[var(--flow-shadow)]">
      ...
      <div className="grid grid-cols-2 border-t border-current/15">
        <div className="border-r border-current/15 px-5 py-4">
          ...
          <p className="mt-1 truncate text-sm font-semibold">{current ? next?.title ?? "ไม่มีงานต่อ" : next?.title ?? "ยังไม่กำหนด"}</p>
