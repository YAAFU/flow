    ...
    <div key={replayKey} className="flex flex-col">
          ...
          <div className="grid grid-cols-[46px_1fr] gap-2.5">
            ...
            <div className="mb-2 rounded-xl border-[1.5px] border-[var(--flow-ink)] px-3 py-2.5">
              <div className="flex items-center gap-1.5">
                ...
                {it.aiAdded && <span className="rounded-full bg-[#eaf4fe] px-1.5 py-0.5 text-[9px] font-semibold text-sky-600">เติมโดย AI</span>}
              </div>
              <div className="mt-0.5 text-[11px] text-neutral-500">{it.placeLabel} · <span className="font-grotesk">{it.start}–{it.end}</span></div>
            </div>
          </div>
        </div>
      ))}
      {riskPoints.map((r, i) => (
        <div key={i} className="grid grid-cols-[46px_1fr] gap-2.5 flow-rise" style={{ animationDelay: `${(items.length + i) * 90}ms` }}>
          ...
          <div className="mb-2 rounded-xl border-[1.5px] border-[var(--flow-ink)] bg-[var(--flow-ink)] px-3 py-2.5 text-white">
